...
test("auto Shenron holds the highest mutation-adjusted DPS eligible unit for Doombringer", () => {
  ...
  const compareBody = source.match(/function Feature\.compareShenronDoombringerCandidates\(left, right\)([\s\S]*?)\nfunction Feature\.getShenronDoombringerCandidates/)?.[1] ?? "";
  ...
  assert.match(source, /doombringerSkipTraits = \{ "Omnipotent", "Doombringer" \}/);
  assert.doesNotMatch(source, /doombringerSkipTraits = \{[^}]*"Corrupted"/);
  assert.match(source, /doombringerLineupDpsTolerance = 0\.10/);
  ...
  assert.match(source, /function Feature\.getShenronDoombringerUnitKey/);
  assert.match(source, /function Feature\.captureShenronDoombringerPlacedTargets/);
  assert.match(source, /function Feature\.getShenronDoombringerLineupPriorityMap/);
  assert.match(source, /function Feature\.getShenronDoombringerLineupPriority/);
  assert.match(source, /function Feature\.compareShenronDoombringerCandidates/);
  ...
  assert.match(compareBody, /Config\.shenron\.doombringerLineupDpsTolerance/);
  assert.match(compareBody, /local dpsGap = math\.abs\(leftDps - rightDps\) \/ math\.max\(math\.max\(leftDps, rightDps\), 1\)/);
  assert.match(compareBody, /if dpsGap > tolerance then[\s\S]*return leftDps > rightDps/);
  assert.match(compareBody, /local leftPriority = tonumber\(left and left\.lineupPriority\) or 0/);
  assert.match(compareBody, /if leftPriority ~= rightPriority then[\s\S]*return leftPriority > rightPriority/);
  assert.match(candidatesBody, /State\.scanUnits\(\)/);
  assert.match(candidatesBody, /local lineupPriorityMap = Feature\.getShenronDoombringerLineupPriorityMap\(\)/);
  ...
  assert.match(candidatesBody, /lineupPriority = Feature\.getShenronDoombringerLineupPriority\(unit, lineupPriorityMap\)/);
  assert.match(candidatesBody, /table\.sort\(candidates/);
  assert.match(candidatesBody, /return Feature\.compareShenronDoombringerCandidates\(left, right\)/);
  assert.match(pickupAllBody, /State\.scanUnits\(\)/);
  assert.match(pickupAllBody, /Feature\.captureShenronDoombringerPlacedTargets\(\)/);
  ...
  assert.match(prepareBody, /local grantedStats = Feature\.computeUnitDoombringerGrantedStats\(unit, candidate\.targetStats\)/);
  assert.match(prepareBody, /grantedDps = grantedDpsText/);
...
test("auto buhara reads wanted food and teleports instead of blind dropping", () => {
  ...
  assert.match(source, /dataPollInterval = 1\.0/);
  assert.match(source, /buharaDataScanAt = 0/);
  ...
  assert.doesNotMatch(source, /BuharaDropFood/);
  assert.doesNotMatch(source, /DropFood/);
  assert.doesNotMatch(source, /dropBuharaFoodIfReady/);
  assert.doesNotMatch(source, /buharaDropAt/);
  ...
  assert.match(source, /function Feature\.hasActiveBuharaProgress/);
  ...
  assert.doesNotMatch(source, /Remote\.fire\("BuharaDropFood"\)/);
  assert.doesNotMatch(source, /Feature\.dropBuharaFoodIfReady\(data\)/);
  ...
  assert.match(source, /function Feature\.giveCarriedBuharaFood/);
  ...
  const dataBody = source.match(/function Feature\.getBuharaData\(\)([\s\S]*?)\nfunction Feature\.getBuharaGuiSlots/)?.[1] ?? "";
  assert.match(dataBody, /Config\.buhara\.dataPollInterval/);
  assert.match(dataBody, /State\.buharaDataScanAt/);
  assert.ok(dataBody.indexOf("State.buharaDataScanAt") < dataBody.indexOf('Remote.invoke("BuharaData")'));

  const activeBody = source.match(/function Feature\.isBuharaEventActive\(\)([\s\S]*?)\nfunction Feature\.hasActiveBuharaProgress/)?.[1] ?? "";
  assert.match(activeBody, /local eventNames = Feature\.getBuharaEventNames\(\)/);
  assert.doesNotMatch(activeBody, /DataSource\.expandSnipeEventNames\(Feature\.getBuharaEventNames\(\)\)/);

  const activeProgressBody = source.match(/function Feature\.hasActiveBuharaProgress\(data\)([\s\S]*?)\nfunction Feature\.shouldRunAutoBuhara/)?.[1] ?? "";
  assert.match(activeProgressBody, /FoodNeeded/);
  assert.match(activeProgressBody, /Feature\.getBuharaWantedFoods\(data\)/);
  assert.match(activeProgressBody, /Feature\.areBuharaRequirementsReady\(data\)/);
  assert.match(activeProgressBody, /PlayerGui:FindFirstChild\("BuharaEvent"\)/);
  ...
  assert.match(shouldRunBody, /local data = Feature\.getBuharaData\(\)/);
  assert.match(shouldRunBody, /Feature\.hasActiveBuharaProgress\(data\)/);
  ...
  const carriedGiveBody = source.match(/function Feature\.giveCarriedBuharaFood\(target, prompt\)([\s\S]*?)\nfunction Feature\.feedBuhara/)?.[1] ?? "";
  assert.match(carriedGiveBody, /Feature\.isCarryingBuharaFood\(\)/);
  assert.match(carriedGiveBody, /Feature\.moveToBuharaFeedPrompt\(target, prompt\)/);
  assert.match(carriedGiveBody, /Feature\.tryBuharaPrompt\(prompt\)/);
  assert.doesNotMatch(carriedGiveBody, /Remote\.fire\("BuharaDropFood"\)/);
  assert.match(carriedGiveBody, /return not Feature\.isCarryingBuharaFood\(\)/);
  ...
  assert.match(feedBody, /local prompt = Feature\.getBuharaFeedPrompt\(target\)/);
...
test("buhara collection and feeding use direct teleport positioning", () => {
  ...
  assert.match(feedBody, /Feature\.giveCarriedBuharaFood\(target, prompt\)/);
  assert.match(carriedGiveBody, /Feature\.tryBuharaPrompt\(prompt\)/);
